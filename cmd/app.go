...
type AppStartArguments struct {
    ...
    ImpersonatedTokenConfigProvider impersonate.TargetPrincipalForProjectProvider
...
func Run(args AppStartArguments) {
    ...
    api := rest.NewAPI(rest.NewAPIArgs{
        ...
        TokenSourceProvider: args.ImpersonatedTokenConfigProvider,
        CredentialsProvider: args.CredentialsProvider,
    })
...
func createBuilder(provider AppStartArguments) *builder.ProcessorBuilder {
    ...
    factories = append(factories, processor.NewCreatingProcessorFactory(provider.SinkGCPProjectProvider, provider.ImpersonatedTokenConfigProvider, provider.CredentialsProvider))
    factories = append(factories, processor.NewListingProcessorFactory(provider.ImpersonatedTokenConfigProvider, provider.CredentialsProvider))
    factories = append(factories, processor.NewUpdatingProcessorFactory(provider.ImpersonatedTokenConfigProvider, provider.CredentialsProvider))
    factories = append(factories, processor.NewGettingProcessorFactory(provider.ImpersonatedTokenConfigProvider, provider.CredentialsProvider))
    factories = append(factories, processor.NewRestoringProcessorFactory(provider.ImpersonatedTokenConfigProvider, provider.CredentialsProvider))
    factories = append(factories, processor.NewCalculatingProcessorFactory(provider.SinkGCPProjectProvider, provider.ImpersonatedTokenConfigProvider))
    factories = append(factories, processor.NewDatasetListingProcessorFactory(provider.SinkGCPProjectProvider, provider.ImpersonatedTokenConfigProvider))
    factories = append(factories, processor.NewBucketListingProcessorFactory(provider.SinkGCPProjectProvider, provider.ImpersonatedTokenConfigProvider))
