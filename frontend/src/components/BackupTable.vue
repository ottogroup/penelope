...
<template>
    ...
    >
        ...
        <template #[`item.source`]="{ item }">
            <template v-if="item.type === 'BigQuery'">
                BigQuery: <a :href="bigqueryDatasetLink(item.project ?? '', item.bigquery_options?.dataset ?? '')">{{ item.bigquery_options?.dataset }}</a>
                <ul>
                    <li v-for="table in item.bigquery_options?.table">
                        Table: <a :href="bigqueryTableLink(item.project ?? '', item.bigquery_options?.dataset ?? '', table)">{{ table }}</a>
                    </li>
                </ul>
            </template>
            <template v-if="item.type === 'CloudStorage'">
                Bucket: <a :href="cloudStorageLink(item.project ?? '', item.gcs_options?.bucket ?? '')">{{ item.gcs_options?.bucket }}</a>
            </template>
        </template> 
    </v-data-table>
</template>