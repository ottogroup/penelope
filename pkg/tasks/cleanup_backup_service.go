...
func (j *cleanupBackupService) handleCloudStorageMirror(ctxIn context.Context, t repository.BackupType) {
    ...
    if err != nil {
        ...
        glog.Info("[START] Deleting old CloudStorage objects")
        err = j.cleanupCloudStorageObjects(ctx, backups)
        if err != nil {
            glog.Warningf("[FAIL] Error deleting old CloudStorage objects: %s", err)
        } else {
            glog.Info("[SUCCESS] Deleting old CloudStorage objects finished")
        }
    }
}
